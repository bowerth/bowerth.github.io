...
#### set back to kernel-4.6.7-300

##### download files from https://kojipkgs.fedoraproject.org/packages/kernel/4.6.7/300.fc24/x86_64/

- kernel-4.6.7-300.fc24.x86_64
- kernel-core-4.6.7-300.fc24.x86_64
- kernel-modules-4.6.7-300.fc24.x86_64
- kernel-devel-4.6.7-300.fc24.x86_64
- kernel-headers-4.6.7-300.fc24.x86_64

copy one rpm file at a time to new dir and install
:   `$ mkdir /tmp/kernel`  
    `$ cd /tmp/kernel`  
    `$ sudo rpm install *.rpm`

in order to install kernel-header, downgrade if necessary
:   `$ sudo rpm downgrade kernel-headers`
...
- `sudo akmods` or `sudo akmods --force`
...
- clone repo [github: hadess: displaylink-rpm](https://github.com/hadess/displaylink-rpm)
- download version of DisplayLink USB Graphics Software for Ubuntu from [displaylink.com](http://www.displaylink.com/downloads/ubuntu) and save to repository
- run `make`
- resulting rpm should created in `x86_64` subfolder

install on specific kernel
:   `$ sudo dkms install evdi/1.2.55`

remove existing installation for specific kernel
:   `$ sudo dkms remove evdi/1.2.55 -k 4.7.4-200.fc24.x86_64/x86_64`

- fork [ssaavedra/displaylink-rpm](https://github.com/ssaavedra/displaylink-rpm)
- fork [ahamilto/displaylink-rpm](https://github.com/ahamilto/displaylink-rpm)
...
- [github: ssaavedra: displaylink-rpm](https://github.com/ssaavedra/displaylink-rpm/releases/)

download and unzip driver, and allow execute `.run` script

#### unbind and re-bind USB ports

- [How do you reset a USB device from the command line?](http://askubuntu.com/questions/645/how-do-you-reset-a-usb-device-from-the-command-line)

`dmesg | grep "usb 1-2"` (left USB port) returns

```
[    0.828443] usb 1-1: new high-speed USB device number 2 using xhci_hcd
[    0.995705] usb 1-1: New USB device found, idVendor=17e9, idProduct=03e0
[    0.995710] usb 1-1: New USB device strings: Mfr=1, Product=2, SerialNumber=3
[    0.995713] usb 1-1: Product: Lenovo LT1421 wide
[    0.995715] usb 1-1: Manufacturer: DisplayLink
[    0.995717] usb 1-1: SerialNumber: 6V9CATN2
```

list drivers
:   `$ cd /sys/bus/pci/drivers/xhci_hcd/ && ls` shows `0000:00:14.0`
